...
export default class SelectCloud extends Component<Props, State> {
  ...
  componentDidMount() {
    const observer = new MutationObserver((mutationList, o) => {
      const haveitem = mutationList
        ...
        .map((rnode) => {
          ...
          isItemClassElements.push(rnode[0].getAttribute('class')?.includes(this.props.options.itemsClass));
          const nodes = rnode[0].querySelectorAll(`*`);

          nodes.forEach((node: { getAttribute: (arg0: string) => string | string[] }) => {
            isItemClassElements.push(node.getAttribute('class')?.includes(this.props.options.itemsClass));
          });
    ...
    const targetNode = document.querySelector(`.${this.props.options.containerClass}`);
    ...
    return (
      <>
        {this.props.selectable && this.state.haveNode && (
          <SelectCloudWorker children={this.props.children} options={this.props.options} />
        )}
        {(!this.props.selectable || !this.state.haveNode) && this.props.children}
        {!this.state.haveNode && this.props.loadingComponent && this.props.loadingComponent}
      </>
    );
  }
}
