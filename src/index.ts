...
export default function SelectWithCloud({ itemsClass, containerClass, cloudStyle, crossEffect, onComplete }: Props) {
  ...
  const mouseMoveHandler = (e: MouseEvent) => {
    ...
    if (e.clientY < startPosition.y - window.scrollY) {
      ...
      cloudSelection.style.height = startPosition.y - window.scrollY - e.clientY + 'px';
    } else {
      cloudSelection.style.bottom = window.scrollY + e.clientY - startPosition.y + 'px';
      cloudSelection.style.height = window.scrollY + e.clientY - startPosition.y + 'px';
    }
  };
