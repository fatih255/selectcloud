...
export default function SelectWithCloud({ itemsClass, containerClass, cloudStyle, crossEffect, onComplete }: Props) {
  if (!runnedSelectWithCloud) {
    ...
    let notes = Array.from(document.querySelectorAll(`.${itemsClass}`)) as HTMLElement[];
    ...
    const mouseDownHandler = (e: MouseEvent) => {
      ...
      notes = Array.from(document.querySelectorAll(`.${itemsClass}`)) as HTMLElement[];
      notes.forEach((note) => {
        if (!intersectionsNotes.includes(note)) {
          note.setAttribute('style', notesInitialStyle);
          note.setAttribute('class', notesInitialClass);
