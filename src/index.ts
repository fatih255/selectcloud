...
export default function SelectWithCloud({ itemsClass, containerClass, cloudStyle, crossEffect, onComplete }: Props) {
  ...
  const mouseMoveHandler = (e: MouseEvent) => {
    if (cloudSelection.offsetHeight + cloudSelection.offsetTop + scrollingYPX > window.innerHeight * 0.7) {
      window.scrollTo({
    ...
    if (e.clientY < startPosition.y) {
      ...
      cloudSelection.style.height = scrollingYPX + startPosition.y - e.clientY + 'px';
    } else {
      cloudSelection.style.height = scrollingYPX + e.clientY - startPosition.y + 'px';
    }
  };
